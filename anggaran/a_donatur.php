<?php
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_GT['del']) {
			if(!getValue("1","t_program","id_donatur='".$_GT['del']."' limit 0,1")) {
				queryDb("delete from t_donatur where id_donatur='".$_GT['del']."'");
		...
		if($_PT['tSimpan']) {
			...
			$errtNama=(!$tNama)?"Data Nama masih kosong":((getValue("1","t_donatur","nama='".$tNama."' and id_donatur<>'".$tId."' limit 1"))?"Nama Donatur sudah terdaftar":"");
			$errtAlamat=(!$tAlamat)?"Data Alamat masih kosong":"";
			
			if(!$errtNama && !$errtAlamat) {				
				if($tId) {
					queryDb("update t_donatur set nama='".$tNama."',alamat='".$tAlamat."',tanggal='".TANGGAL."' where id_donatur='".$tId."'");
				}
				else {
					$tId=substr(((getValue("id_donatur","t_donatur","1=1 order by id_donatur desc limit 0,1")*1)+1001),1,3);
					queryDb("insert into t_donatur(id_donatur,nama,alamat,tanggal) values('".$tId."','".$tNama."','".$tAlamat."','".TANGGAL."')");
				}
...
<div id="list">
	...
	$row=50;
	$page=8;
	$sql="select d.id_donatur,d.nama,d.alamat,ifnull((select sum(nominal) from t_anggaran where id_donatur=d.id_donatur and concat(id_unit1,'.',id_unit2,'.',id_unit3) in ('".implode("','",array_keys($_UNITS))."')),0) as nominal,if(min(p.id_program) is null,1,0) as stat_delete from t_donatur d
					left join t_program p on p.id_donatur=d.id_donatur
	...
	if(is_array($paging)) {		
		...
		while($b=mysql_fetch_array($a)) {
			...
			echo "<ul id=\"".$b['id_donatur']."\" onclick=\"listFocus(this,'edit=1,del=".$b['stat_delete']."')\" ondblclick=\"_URI['id1']='".$b['id_donatur']."';_URI['order1']='".$_GT['order']."';goAddress('id1,order1,hal,sort,search','a_program.php')\"><li>".$paging['start'].".</li><li>".$b['id_donatur']."</li><li>".$b['nama']."</li><li>".$b['alamat']."</li><li>".showRupiah($b['nominal'])."</li></ul>";
		
			if($_GT['edit']==$b['id_donatur']) $jsEdit="listFocus(elm('".$b['id_donatur']."'),'edit=1,del=".$b['stat_delete']."');";
		}
	}
	?>
</div>
<div id="titleBottom">
	<ol>
    	<li style="width:auto;" class="l">
            ...
            <input type="text" style="width:120px;" onblur="_URI['search']=this.value;" onblur="_URI['search']=this.value;" onmouseout="_URI['search']=this.value;" value="<?=$_GT['search']?>" />
    	...
    	<li class="r" style="width:auto;">
    	  <button class="icon-new" onclick="elm('tIdDonatur').value='<?=substr(((getValue("id_donatur","t_donatur","1=1 order by id_donatur desc limit 0,1")*1)+1001),1,3)?>';newData('tId,tNama,tAlamat');">TAMBAH</button>
    	  <button id="btn-edit" class="icon-edit disabled" onclick="editData('tId,tNama,tIdDonatur,tAlamat');">EDIT</button>
