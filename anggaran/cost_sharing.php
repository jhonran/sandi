	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_PT['tSimpan']) {	
			...
			if(is_array($_PT['tNominal'])) {			
				queryDb("delete from t_costsharing where concat(id_unit1,'.',id_unit2,'.',id_unit3)='".$_SESSION['unit']."'");
				
				reset($_PT);
				while(list($a,$b)=each($_PT['tNominal'])) {
					if($b>0 && $b<101) {
						$tId=(getValue("id_costsharing","t_costsharing","1=1 order by id_costsharing desc limit 0,1")*1)+1;
						queryDb("insert into t_costsharing(id_costsharing,id_unit1,id_unit2,id_unit3,id_gl4,persen,tanggal,id_user) 
									values('".$tId."','".$_UNIT['id_unit1']."','".$_UNIT['id_unit2']."','".$_UNIT['id_unit3']."','".$a."','".clearRupiah($b)."','".TANGGAL."','".$_SESSION['user']."')");
					}
				}
			}
		...
		$sql="select g.id_gl1,g.id_gl2,g.id_gl3,g.id_gl4,g1.nama as nama1,g2.nama as nama2,g3.nama as nama3,g.nama as nama4,ifnull(d.persen,0) as nilai
						from t_gl4 g
							...
							left join t_costsharing d on d.id_gl4=g.id_gl4 and concat(d.id_unit1,'.',d.id_unit2,'.',d.id_unit3)='".$_SESSION['unit']."'
						where g1.balance='LR' and g1.db_cr='D'
		...
		while($b=mysql_fetch_array($a)) {
			...
			$_S[$b['id_gl1']][$b['id_gl2']][$b['id_gl3']][$b['id_gl4']]="<input type=\"text\" name=\"tNominal[".$b['id_gl4']."]\" value=\"".showRupiah2($b['nilai'])."\" maxlength=\"3\" onkeyup=\"valnominal(this);\" />";
		}
?>
...
<style type="text/css">
#titleTop ol li:nth-child(2) , #list ul li:nth-child(3) { width:95px;text-align:center; }
#list ul li input[type=text] { width:80px;text-align:right; }
