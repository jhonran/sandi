<?php
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_GT['delete']) {
			if(!getValue("1","php_giat","id_program='".$_GT['delete']."' and id_donatur='".$_GT['id1']."' limit 0,1")) {
				queryDb("delete from php_program where id_program='".$_GT['delete']."' and id_donatur='".$_GT['id1']."'");
		...
		if($_PT['tSimpan']) {
			...
			$errtNama=(!$tNama)?"Data Nama masih kosong":((getValue("1","php_program","nama='".$tNama."' and id_program<>'".$tId."' and id_donatur='".$_GT['id1']."' limit 1"))?"Nama Program sudah terdaftar":"");
			$errtTanggal=(!balikTanggal($tTanggal))?"Format Tanggal masih salah (DD/MM/YYYY)":((calculateDate(balikTanggal($tTanggal),TANGGAL2)<0)?"Tanggal tidak boleh lebih kecil dari sekarang":"");
				
			if(!$errtNama && !$errtAlamat && !$errtTanggal) {				
				if($tId) {
					queryDb("update php_program set nama='".$tNama."',expired='".balikTanggal($tTanggal)."',tanggal='".TANGGAL."' where id_program='".$tId."' and id_donatur='".$_GT['id1']."'");
				}
				else {
					$tId=substr(((getValue("id_program","php_program","id_donatur='".$_GT['id1']."' order by id_program desc limit 0,1")*1)+101),1,2);
					queryDb("insert into php_program(id_program,id_donatur,nama,expired,tanggal) values('".$tId."','".$_GT['id1']."','".$tNama."','".balikTanggal($tTanggal)."','".TANGGAL."')");
				}
...
<div id="list">
	<?php
	$paging['start']=0;
	$sql="select p.id_program,p.nama,p.expired,ifnull((select sum(nominal) from php_detail where id_donatur=p.id_donatur and id_program=p.id_program),0) as nominal,if(min(g.id_giat) is null,1,0) as stat_delete from php_program p
					left join php_giat g on g.id_program=p.id_program and g.id_donatur=p.id_donatur		
	...
	while($b=mysql_fetch_array($a)) {
		...
		echo "<ul id=\"".$b['id_program']."\" onclick=\"listFocus(this,1,".$b['stat_delete'].")\" ondblclick=\"_URI['id2']='".$b['id_program']."';_URI['order2']='".$_GT['order']."';goAddress('id1,hal,sort,search,order1,id2,order2','giat.php')\"><li>".$paging['start'].".</li><li>".$_GT['id1'].".".$b['id_program']."</li><li>".$b['nama']."</li><li>".tglIndo($b['expired'],2)."</li><li>".showRupiah($b['nominal'])."</li></ul>";
	
		if($_GT['edit']==$b['id_program']) $jsEdit="listFocus(elm('".$b['id_program']."'),1,".$b['stat_delete'].");";
	}
	?>
</div>
<div id="titleBottom">
	<ol>
    	<li class="r" style="width:auto;">
    	  <button class="icon-new" onclick="elm('tIdProgram').value='<?=$_GT['id1'].".".substr(((getValue("id_program","php_program","id_donatur='".$_GT['id1']."' order by id_program desc limit 0,1")*1)+101),1,2)?>';setTanggal('<?=date("d/m/Y")?>');newData('tId,tNama');">TAMBAH</button>
    	  <button id="bEdit" class="icon-edit disabled" onclick="editData('tId,tNama,tTanggal,tIdProgram');setTanggal(elm('tTanggal').value)">EDIT</button>
    	  <button id="bDelete" class="icon-delete disabled" onclick="deleteData('delete,order,id1,hal,sort,search,order1');">HAPUS</button>
        </li>
    </ol>
</div>
