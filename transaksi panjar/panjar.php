<?php
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_GT['del']) {
			if($statHapusTrans==1 && getValue("1","t_panjar p","p.no_trans='".$_GT['del']."' and p.no_trans2='' and tgl_trans>'".$lastPosting."' limit 1")) {
				...
				*/
				...
				if(mysql_affected_rows()>0) {
					queryDb("delete from t_panjar where no_trans='".$_GT['del']."' and tgl_trans>'".$lastPosting."'");
				}
		...
		if($_PT['tSimpan']) {
			...
			$iPegawai=$_PT['iPegawai'];
			$nPegawai=$_PT['nPegawai'];
			...
			$errnPegawai=(!$nPegawai)?$_IST['pegawai.php']." masih kosong":"";
			$errtExpired=(!balikTanggal($tExpired))?"Format Tanggal masih salah (DD/MM/YYYY)":((dateDiff(TANGGAL2,balikTanggal($tExpired))>0)?"Tanggal tidak boleh lebih kecil dari sekarang":"");
			//$errnAkunD=(!$iAkunD)?$_IST['gl1.php']." Debit masih kosong":((!isset($_AKUN['DB'][$iAkunD]))?$_IST['gl1.php']." Debit tidak terdaftar":((getValue("1","t_panjar","id_gl4='".$iAkunD."' and no_trans3='' limit 1")==1)?$_IST['gl1.php']." belum melakukan penyelesaian Panjar":""));
			...
			if(!$errtTanggal && !$errtNoBukti && !$errnPegawai && !$errtExpired && !$errnAkunD && !$errnAkunC && !$errtBatas && !$errtNominal) {				
				if($tId) {
					...
					$tExpired=balikTanggal($tExpired);
					
					queryDb("insert into t_panjar(id_unit1,id_unit2,id_unit3,id_pegawai,nm_pegawai,tgl_expired,id_gl4,tgl_trans,no_trans,no_bukti,keterangan,nominal,tanggal,id_user,tgl_trans2,no_trans2,no_bukti2,keterangan2,nominal2,tanggal2,id_user2,tgl_trans3,no_trans3,no_bukti3,keterangan3,nominal3,tanggal3,id_user3)
									values('','','','".$iPegawai."','".$nPegawai."','".$tExpired."','".$iAkunD."','".$tTanggal."','".$tIdTrans."','".$tNoBukti."','".$tDesc."','".clearRupiah($tNominal)."','".TANGGAL."','".$_SESSION['user']."','".$tTanggal."','','','','0','".TANGGAL."','".$_SESSION['user']."','".$tTanggal."','','','','0','".TANGGAL."','".$_SESSION['user']."')");
...
#titleTop ol li:nth-child(5) , #list ul li:nth-child(5) { width:250px;text-align:center; }
#titleTop ol li:nth-child(7) , #list ul li:nth-child(7) { width:120px; }
#titleTop ol li:nth-child(8) , #list ul li:nth-child(8) { width:130px;text-align:center; }
#titleTop ol li { text-align:center; }
#list ul li:nth-child(7) { text-align:right; }
          ...
          <form action="" target="_self" method="post" onsubmit="showFade('load');">
              <table cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td>:</td>
                  <td>
                      <input type="hidden" name="iPegawai" id="iPegawai" value="<?=htmlentities($iPegawai)?>" />
                      <input name="nPegawai" type="text" id="nPegawai" maxlength="200" value="<?=htmlentities($nPegawai)?>" autocomplete="off" onkeyup="goFramePopup(this,'Pegawai','pegawai');hideFade('errnPegawai');" onfocus="framePopupFocus('Pegawai')" onblur="if(!elm('iPegawai').value) elm('iPegawai').value='-'; framePopupBlur('Pegawai')" />
                      <div class="framePopup"><img id="imgPegawai" src="../images/loader.gif" /><iframe id="fPegawai"></iframe></div>
                    <div id="errnPegawai" class="err"><?=$errnPegawai?></div>
                  </td>
                </tr>
                <tr>
                  <td>BATAS WAKTU</td>
                  <td>:</td>
                  <td>
                    <input type="text" name="tExpired" id="tExpired" maxlength="10" onkeydown="return DateFormat(this,event.keyCode)" autocomplete="off" required="required" onkeyup="hideFade('errtExpired')" />
                    <div id="errtExpired" class="err"><?=$errtExpired?></div>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td>:</td>
                  <td>
                    <input type="text" name="tNominal" id="tNominal" maxlength="20" required="required" value="<?=htmlentities($tNominal)?>" onkeyup="hideFade('errttNominal');valnominal(this);" />
                    <div id="errttNominal" class="err"><?=$errtNominal?></div>
                  </td>
                </tr>
                <tr>
                  <td>KETERANGAN</td>
...
<div id="titleTop">
	...
	<ol class="title">
    	<li>NO</li>
        <?php        
        $listTitle=array("x.tgl_trans#Tanggal","x.no_trans#No Trans","x.no_bukti#No Bukti","u.nama#Unit Penerima","x.keterangan#Keterangan","x.nominal#Nominal","#Status");
...
<div id="list">
	...
	$row=50;
	$page=8;
	$sql="SELECT x.tgl_trans,x.no_trans,x.no_bukti,x.keterangan,x.nominal,if(x.tgl_trans>'".$lastPosting."',if(j.no_trans2='',1,0),0) as stat_delete,if(j.tgl_expired<'".TANGGAL."',1,0) as stat_expired,
						concat(j.id_unit1,'.',j.id_unit2,'.',j.id_unit3) as id_unit,if(u.nama is null,concat('[',j.id_gl4,'] ',g4.nama),concat('[',j.id_unit1,'.',j.id_unit2,'.',j.id_unit3,'] ',u.nama)) as unit,j.id_pegawai,j.nm_pegawai,j.tgl_expired,if(j.no_trans2='','<span class=\'err\'>Belum diterima</span>',if(j.no_trans3='','<span class=\'err\'>Belum diselesaikan</span>','Sudah diselesaikan')) as stat
					from (
					...
					inner join t_panjar j on j.no_trans=x.no_trans
					inner join t_gl4 g4 on g4.id_gl4=j.id_gl4
					left join t_unit3 u on u.id_unit1=j.id_unit1 and u.id_unit2=j.id_unit2 and u.id_unit3=j.id_unit3
	...
	if(is_array($paging)) {		
		...
		while($b=mysql_fetch_array($a)) {
			...
			echo "<ul id=\"".$b['no_trans']."\" ".(($b['stat_expired']==1)?"class=\"g\"":"")." onclick=\"listFocus(this,'edit=1,del=".$b['stat_delete'].",search=1')\"><li>".$paging['start'].".</li><li>".tglIndo(balikTanggal($b['tgl_trans']),2)."</li><li>".$b['no_trans']."</li><li>".$b['no_bukti']."</li>
					<li>".$b['unit']."</li><li>".$b['keterangan']."</li><li>".showRupiah2($b['nominal'])."</li><li>".$b['stat']."</li></ul>";
...
<div id="titleBottom">
	<ol>
    	...
    	<li class="r" style="width:auto;">
    	  <button id="btn-new" class="icon-new" onclick="elm('tIdTrans').value='<?=NOFAK?>';setTanggal('<?=date("d/m/Y")?>','<?=dateInterval(TANGGAL2,0,0,15,0,0,0,"d/m/Y")?>');newData('tId,tNoBukti,tDesc');">TAMBAH</button>
...
<script language="javascript">
	...
	function setTanggal(v1,v2) {
		...
		$(function() {
			$("#tExpired").datepicker();
			$("#tExpired").datepicker("option","dateFormat","dd/mm/yy");
			$("#tExpired").datepicker("setDate",v2);
		});
	}
		
	<?php
		if($tTanggal && $tExpired) echo "setTanggal('".$tTanggal."','".$tExpired."');";
	?>
	...
	hideFade("load","<?=($errtTanggal || $errtNoBukti || $errnPegawai || $errtExpired || $errnAkunD || $errnAkunC || $errtBatas || $errtNominal)?"showFade('input')":""?>");
