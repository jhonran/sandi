...
<?php 
	$a=queryDb("select g.id_gl4,concat('[',g.id_gl4,'] ',g.nama) as nm from t_glu g
							inner join t_mtrans_gl m on m.id_gl4=g.id_gl4 and concat(m.mutasi,'.',m.id_mtrans)='".$_GT['r']."'
						where concat('[',g.id_gl4,'] ',g.nama) like '%".$_GT['s']."%' and concat(g.id_unit1,'.',g.id_unit2,'.',g.id_unit3)='".$_GT['q']."'
