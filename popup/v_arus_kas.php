...
$saldoAwal=getValue("sum(nominal_d-nominal_c)","t_glt","id_aruskas='00' and concat(id_unit1,'.',id_unit2,'.',id_unit3) in ('".str_replace("#","','",$tUnit)."') and tgl_trans<'".balikTanggal($_GT['periode1'])." 00:00:00'");
...
$statTemp=0;
$nilaiTemp=0;
$sql="select y.no_trans,if(y.id_aruskas='','100',y.id_aruskas) as id_aruskas,if(y.id_aruskas_sub='','100',y.id_aruskas_sub) as id_aruskas_sub,y.id_gl4,ifnull(g.nama,'') as nama,y.nilai,if(y.nilai>0,'1','-1') as stat from (
				...
				left join t_gl4 g on g.id_gl4=y.id_gl4
    	...
    	<td>&nbsp;<br />
			...
			if(is_array($_N3)) {
				...
				while(list($a,$b)=each($_N3)) {
					echo "<tr><td>&nbsp;<br /><strong>".$_A[$a]."</strong></td><td>&nbsp;</td><td class='r'>&nbsp;<br /><strong>".numbAcc($_N[$a])."</strong></td></tr>";
										
					ksort($b);
					while(list($aa,$bb)=each($b)) {
						echo "<tr><td>- &nbsp; ".$_A2[$a][$aa]."</td><td>&nbsp;</td><td class='r'>".numbAcc($_N2[$a][$aa])."</td></tr>";
					}
				}
			}	 
            ?>
            	...
            	<tr>
                	...
                	<td class='r bg'><strong><?=numbAcc(array_sum($_N)+$saldoAwal)?></strong></td>
            	</tr>
             </table><br />&nbsp;
        </td>
    </tr>
