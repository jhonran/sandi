...
<?php 
	$a=queryDb("select d.id_donatur,concat('[',d.id_donatur,'] ',d.nama) as nm from t_donatur d
							inner join t_anggaran a on a.id_donatur=d.id_donatur and concat(a.id_unit1,'.',a.id_unit2,'.',a.id_unit3)='".$_SESSION['unit']."'
							inner join t_program p on p.id_donatur=d.id_donatur and p.id_program=a.id_program and p.expired>='".balikTanggal($_GT['v'])."'
						where concat('[',d.id_donatur,'] ',d.nama) like '%".$_GT['s']."%'
						group by d.id_donatur,nm 
						order by d.id_donatur limit 50");
						
	if(!mysql_num_rows($a)) echo "<div class='err'>Data tidak ditemukan</div>";
	
	while($b=mysql_fetch_array($a)) {
	?>
		<ul><li onclick="<?php echo "sendText('".$b['id_donatur']."','".$b['nm']."')"; ?>"><?php echo $b['nm']; ?></li></ul>
<?php } ?>
</div>
<script language="javascript">
	parent.hideFade('imgDonatur');
	parent.showFade('fDonatur');
	
	function sendText(a,b) {
		parent.elm('iDonatur').value=a;
		parent.elm('nDonatur').value=b;
		setTimeout("parent.hideFade('fDonatur');",100);
	}
</script>
</body>
</html>
