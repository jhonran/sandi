...
$sql="select x.id_aset,sum(x.akum_susut1) as akum_susut1,sum(x.akum_susut2_d) as akum_susut2_d,sum(x.akum_susut2_k) as akum_susut2_k from (
					select a.id_aset,
							if(s.tgl_trans<'".balikTanggal($_GT['periode1'])." 00:00:00',s.nominal,0) as akum_susut1,
							if(s.tgl_trans>='".balikTanggal($_GT['periode1'])." 00:00:00' and s.nominal>=0,s.nominal,0) as akum_susut2_d,
							if(s.tgl_trans>='".balikTanggal($_GT['periode1'])." 00:00:00' and s.nominal<0,(s.nominal*-1),0) as akum_susut2_k 
						from t_aset a
							inner join t_susut s on s.id_aset=a.id_aset and s.tgl_trans<='".balikTanggal($_GT['periode2'])." 23:59:59'
						where a.tgl_perolehan<='".balikTanggal($_GT['periode2'])." 23:59:59' and concat(a.id_unit1,'.',a.id_unit2,'.',a.id_unit3) in ('".str_replace("#","','",$tUnit)."') and a.jenis='".$_GT['jenis']."' ".$where_hibah." and a.status='1'
				) x
				group by x.id_aset";
...
$a=queryDb($sql);
while($b=mysql_fetch_array($a)) {
	$_D[$b['id_aset']]['akum_susut']+=($b['akum_susut1']+($b['akum_susut2_d']-$b['akum_susut2_k']));
	$_D[$b['id_aset']]['akum_susut1']+=$b['akum_susut1'];
	$_D[$b['id_aset']]['akum_susut2']+=($b['akum_susut2_d']-$b['akum_susut2_k']);
	$_D[$b['id_aset']]['akum_susut2_d']+=$b['akum_susut2_d'];
	$_D[$b['id_aset']]['akum_susut2_k']+=$b['akum_susut2_k'];
	$_D[$b['id_aset']]['nilai_buku']-=($b['akum_susut1']+($b['akum_susut2_d']-$b['akum_susut2_k']));
        	...
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					while(list($a,$b)=each($_D)) {
						if($idAsetKategori!=$_D[$a]['id_aset_kategori']) {
                            ...
                            <tr>
                                ...
                                <td class="r"><strong><?=numbAcc($ttl2)?></strong></td>
						...
						$ttl4+=$b['akum_susut1'];
						$ttl5+=$b['nilai2_d'];
						$ttl6+=$b['akum_susut2_d'];
						$ttl7+=$b['nilai2_k'];
						$ttl8+=$b['akum_susut2_k'];
						$ttl9+=$b['akum_susut2'];
						$ttl10+=$b['akum_susut'];
						$ttl11+=$b['nilai_buku']; // nilai buka s/d periode 2
						$ttl12+=$b['nilai_buku1']; // nilai buku s/d periode 1
						...
						$total5+=$b['nilai2_d'];
						$total6+=$b['akum_susut2_d'];
						$total7+=$b['nilai2_k'];
						$total8+=$b['akum_susut2_k'];
						$total9+=$b['akum_susut2'];
						$total10+=$b['akum_susut'];
						$total11+=$b['nilai_buku'];
						...
						<tr>
						  ...
						  <td class="r"><?=numbAcc($_D[$a]['nilai1'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['akum_susut1'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['nilai2_d'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['akum_susut2_d'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['nilai2_k'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['akum_susut2_k'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['akum_susut2'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['akum_susut'])?></td>
						  <td class="r"><?=numbAcc($_D[$a]['nilai_buku'])?></td>
                ...
                <tr>
                	...
                	<td colspan="3" class="bg"><strong>TOTAL : </strong></td>
                  	...
                  	<td class="r bg"><strong><?=numbAcc($total2)?></strong></td>
