<?php
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		$no=0;
		$a=queryDb("select keterangan,jabatan,pegawai from t_lap_tandatangan where url='".$_SERVER['QUERY_STRING']."' order by id");
		...
		if($_PT['tSimpan']) {
			queryDb("delete from t_lap_tandatangan where url='".$_SERVER['QUERY_STRING']."'");
			
			for($no=1;$no<=5;$no++) {
				if(($_PT['tKeterangan'][$no] && $_PT['nPegawai'][$no]) || ($_PT['tJabatan'][$no] && $_PT['nPegawai'][$no])) {
					queryDb("insert into t_lap_tandatangan(url,keterangan,jabatan,pegawai) values('".$_SERVER['QUERY_STRING']."','".$_PT['tKeterangan'][$no]."','".$_PT['tJabatan'][$no]."','".$_PT['nPegawai'][$no]."')");
				}
			}
		}
?>
