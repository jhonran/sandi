<?php
	...
	$titleHTML=getValue("s.judul","t_user u
											...
											inner join t_menu_sub s on s.id_menu_sub=h.id_menu_sub and s.status='1' and s.page='lcost_sharing.php'",
...
$sql="select x.id_gl1,x.id_gl2,x.id_gl3,x.id_gl4,max(nm1) as nm1,g2.nama as nm2,g3.nama as nm3,g4.nama as nm4,sum(x.nilai_r) as nilai_r,sum(x.nilai) as nilai,sum(x.nilai_r2) as nilai_r2,max(x.nilai_p) as nilai_p
		from (
			(select u.id_gl1,u.id_gl2,u.id_gl3,u.id_gl4,g1.nama as nm1,0 as nilai_r,sum((u.nominal_d-u.nominal_c)*if(g1.db_cr='D',1,-1)) as nilai,0 as nilai_r2,0 as nilai_p from t_glt u
										inner join t_gl1 g1 on g1.id_gl1=u.id_gl1
									where concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3) in ('".str_replace("#","','",$tUnit)."') ".$wsql1." and g1.balance='LR' and g1.db_cr='D'
									group by u.id_gl1,u.id_gl2,u.id_gl3,u.id_gl4)
			UNION
			(select g4.id_gl1,g4.id_gl2,g4.id_gl3,u.id_gl4,g1.nama as nm1,sum(nominal) as nilai_r,0 as nilai,0 as nilai_r2,0 as nilai_p from t_danaanggaran u
										inner join t_gl4 g4 on g4.id_gl4=u.id_gl4
										inner join t_gl1 g1 on g1.id_gl1=g4.id_gl1
									where concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3) in ('".str_replace("#","','",$tUnit)."') ".$wsql2." and g1.balance='LR' and g1.db_cr='D'
									group by g4.id_gl1,g4.id_gl2,g4.id_gl3,u.id_gl4)
			UNION
			(select u.id_gl1,u.id_gl2,u.id_gl3,u.id_gl4,g1.nama as nm1,0 as nilai_r,0 as nilai,sum((u.nominal_d-u.nominal_c)*if(g1.db_cr='D',1,-1)) as nilai_r2,0 as nilai_p from t_glt u
										inner join t_gl1 g1 on g1.id_gl1=u.id_gl1
									where concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3)='01.01.01' /* not in (select distinct concat(id_unit1,'.',id_unit2,'.',id_unit3) from t_costsharing) */ ".$wsql1." and g1.balance='LR' and g1.db_cr='D'
									group by u.id_gl1,u.id_gl2,u.id_gl3,u.id_gl4)
			UNION
			(select g4.id_gl1,g4.id_gl2,g4.id_gl3,u.id_gl4,g1.nama as nm1,0 as nilai_r,0 as nilai,0 as nilai_r2,max(persen) as nilai_p from t_costsharing u
										inner join t_gl4 g4 on g4.id_gl4=u.id_gl4
										inner join t_gl1 g1 on g1.id_gl1=g4.id_gl1
									where concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3) in ('".str_replace("#","','",$tUnit)."') and g1.balance='LR' and g1.db_cr='D'
									group by g4.id_gl1,g4.id_gl2,g4.id_gl3,u.id_gl4)
...
while($b=mysql_fetch_array($a)) {
	...
	$_N1['s'][$b['id_gl1']]+=$b['nilai_r2']*$b['nilai_p']/100;
    ...
    <tr>
    	<td>
        	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="nb">
            	...
            	<td class="c"><strong>COST SHARING</strong></td>
            	<td class="c"><strong>TOTAL</strong></td>
            	</tr>
			<?php
			if(is_array($_G1)) {
				reset($_G1);
				...
				while(list($a,$b)=each($_G1)) {
					echo "<tr><td class='bg'>".$a." &nbsp; ".$b."</td><td class='r bg'>".numbAcc($_N1['a'][$a])."</td><td class='r bg'>".numbAcc($_N1['d'][$a])."</td><td class='r bg'>".numbAcc($_N1['s'][$a])."</td><td class='r bg'>".numbAcc($_N1['d'][$a]+$_N1['s'][$a])."</td></tr>";
					reset($_G2);
					reset($_N2);
					while(list($aa,$bb)=each($_G2[$a])) {
						echo "<tr><td>".$aa." &nbsp; ".$bb."</td><td class='r'>".numbAcc($_N2['a'][$a][$aa])."</td><td class='r'>".numbAcc($_N2['d'][$a][$aa])."</td><td class='r'>".numbAcc($_N2['s'][$a][$aa])."</td><td class='r'>".numbAcc($_N2['d'][$a][$aa]+$_N2['s'][$a][$aa])."</td></tr>";
						if($_GT['level']>=3) {
							reset($_G3);
							reset($_N3);
							while(list($aaa,$bbb)=each($_G3[$a][$aa])) {
								echo "<tr><td class='bg'>".$aaa." &nbsp; ".$bbb."</td><td class='r bg'>".numbAcc($_N3['a'][$a][$aa][$aaa])."</td><td class='r bg'>".numbAcc($_N3['d'][$a][$aa][$aaa])."</td><td class='r bg'>".numbAcc($_N3['s'][$a][$aa][$aaa])."</td><td class='r bg'>".numbAcc($_N3['d'][$a][$aa][$aaa]+$_N3['s'][$a][$aa][$aaa])."</td></tr>";
								if($_GT['level']>=4) {
									reset($_G4);
									reset($_N4);
									while(list($aaaa,$bbbb)=each($_G4[$a][$aa][$aaa])) {
										echo "<tr><td>".$aaaa." &nbsp; ".$bbbb."</td><td class='r'>".numbAcc($_N4['a'][$a][$aa][$aaa][$aaaa])."</td><td class='r'>".numbAcc($_N4['d'][$a][$aa][$aaa][$aaaa])."</td><td class='r'>".numbAcc($_N4['s'][$a][$aa][$aaa][$aaaa])."</td><td class='r'>".numbAcc($_N4['d'][$a][$aa][$aaa][$aaaa]+$_N4['s'][$a][$aa][$aaa][$aaaa])."</td></tr>";
									}
								}
							}
						}				
