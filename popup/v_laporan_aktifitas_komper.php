...
$sql="select if(g1.id_gl1=".$idAset.",t.id_gl4,t.id_glx) as id_gls,t.id_gl1,sum((t.nominal_d-t.nominal_c)*if(g1.db_cr='D',1,-1)) as nilai from t_glt t
						...
						where concat(t.id_unit1,'.',t.id_unit2,'.',t.id_unit3) in ('".str_replace("#","','",$tUnit)."') and t.tgl_trans<'".balikTanggal($_GT['awal'])." 00:00:00'
...
/*$sql="select ifnull(t.id_glx,'') as id_glx,u.id_gl1,u.id_gl2,u.id_gl3,u.id_gl4,g1.nama as nm1,g2.nama as nm2,g3.nama as nm3,u.nama as nm4,sum((t.nominal_d-t.nominal_c)*if(g1.db_cr='D',1,-1)) as nilai from t_glu u
							...
							left join t_glt t on concat(t.id_unit1,'.',t.id_unit2,'.',t.id_unit3)=concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3) and (t.id_gl4=u.id_gl4 or t.id_glx=u.id_gl4) and t.tgl_trans between '".balikTanggal($_GT['awal'])." 00:00:00' and '".$awal2." 23:59:59'
...
$sql="select id_glx,if(id_gl1=3,if((jumlah*perkalian)<0,5,4),id_gl1) as id_gl1,id_gl2,id_gl3,id_gl4,nm1,nm2,nm3,nm4,if(id_gl1=3,abs(jumlah*perkalian),(jumlah*perkalian)) as nilai from (
					select id_glx,id_gl1,id_gl2,id_gl3,id_gl4,max(nm1) as nm1,max(nm2) as nm2,max(nm3) as nm3,max(nm4) as nm4,sum(jumlah) as jumlah,sum(perkalian) as perkalian from (
								...
								(select if(t.id_glx='',t.id_gl4,t.id_glx) as id_glx,t.id_gl1,t.id_gl2,t.id_gl3,t.id_gl4,'' as nm1,'' as nm2,'' as nm3,'' as nm4,sum(t.nominal_d-t.nominal_c) as jumlah,0 as perkalian from t_glt t 
										where concat(t.id_unit1,'.',t.id_unit2,'.',t.id_unit3) in ('".str_replace("#","','",$tUnit)."') and (t.id_glx<>'' or t.id_gl1='".$idAset."') and t.tgl_trans between '".balikTanggal($_GT['awal'])." 00:00:00' and '".$awal2." 23:59:59'
...
while($b=mysql_fetch_array($a)) {
	...
	$_M1[$b['id_glx']][$b['id_gl1']]+=$b['nilai'];
	$_M2[$b['id_glx']][$b['id_gl1']][$b['id_gl2']]+=$b['nilai'];
	$_M3[$b['id_glx']][$b['id_gl1']][$b['id_gl2']][$b['id_gl3']]+=$b['nilai'];
	$_M4[$b['id_glx']][$b['id_gl1']][$b['id_gl2']][$b['id_gl3']][$b['id_gl4']]+=$b['nilai'];
	
	$_ZM1[$b['id_gl1']]+=$b['nilai'];
	$_ZM2[$b['id_gl1']][$b['id_gl2']]+=$b['nilai'];
	$_ZM3[$b['id_gl1']][$b['id_gl2']][$b['id_gl3']]+=$b['nilai'];
	$_ZM4[$b['id_gl1']][$b['id_gl2']][$b['id_gl3']][$b['id_gl4']]+=$b['nilai'];
}
           	...
           	<?php
				if(is_array($_TMP)) {
					echo "<tr>
								...
								<td>&nbsp;</td>
								<td class='bg c' colspan='".(count($_TMP)+1)."'><strong>".tglIndo(balikTanggal($_GT['awal']),2)." s/d ".tglIndo($awal2,2)."</strong></td>
							</tr>";
					...
					echo "<td class='c'><strong>TOTAL</strong></td><tr>";
					...
					if(is_array($_G1)) {
						...
						while(list($a,$b)=each($_G1)) {
							...
							reset($_TMP);							
							while(list($x,$y)=each($_TMP)) {
								echo "<td class='r'>&nbsp;<br /><strong>".numbAcc($_M1[$x][$a])."</strong></td>";
							}
							echo "<td class='r'>&nbsp;<br /><strong>".numbAcc($_ZM1[$a])."</strong></td></tr>";
							...
							while(list($aa,$bb)=each($_G2[$a])) {
								...
								reset($_TMP);							
								while(list($x,$y)=each($_TMP)) {
									echo "<td class='r'>".numbAcc($_M2[$x][$a][$aa])."</td>";
								}
								echo "<td class='r'>".numbAcc($_ZM2[$a][$aa])."</td></tr>";*/
								...
								while(list($aaa,$bbb)=each($_G3[$a][$aa])) {
									...
									reset($_TMP);							
									while(list($x,$y)=each($_TMP)) {
										echo "<td class='r'>".numbAcc($_M3[$x][$a][$aa][$aaa])."</td>";
									}
									echo "<td class='r'>".numbAcc($_ZM3[$a][$aa][$aaa])."</td></tr>";
									...
									while(list($aaaa,$bbbb)=each($_G4[$a][$aa][$aaa])) {
										...
										reset($_TMP);							
										while(list($x,$y)=each($_TMP)) {
											echo "<td class='r'>".numbAcc($_M4[$x][$a][$aa][$aaa][$aaaa])."</td>";
										}
										echo "<td class='r'>".numbAcc($_ZM4[$a][$aa][$aaa][$aaaa])."</td></tr>";*/
					...
					echo "<tr><td colspan='".((count($_TMP)*2)+4)."'>&nbsp;</td></tr>";
					...
					reset($_TMP);							
					while(list($a,$b)=each($_TMP)) {
						echo "<td class='bg r'><strong>".numbAcc($_M1[$a][4]-$_M1[$a][5])."</strong></td>";
					}
					echo "<td class='bg r'><strong>".numbAcc($_ZM1[4]-$_ZM1[5])."</strong></td></tr>";
					...
					reset($_TMP);							
					while(list($a,$b)=each($_TMP)) {
						echo "<td class='r'>".numbAcc($_M1[$a][4]-$_M1[$a][5])."</td>";
					}
					echo "<td class='r'>".numbAcc($_ZM1[4]-$_ZM1[5])."</td></tr>";
					//----------------------------------------------------------------------------
					echo "<tr><td>Aset Bersih Awal Periode :</td>";
					reset($_TMP);							
					while(list($a,$b)=each($_TMP)) {
						echo "<td class='r'>".numbAcc(($_M1[$a][4]-$_M1[$a][5])+($_A1[$a][4]-$_A1[$a][5]))."</td>";
					}
					echo "<td class='r'>".numbAcc(($_ZM1[4]-$_ZM1[5])+($_ZA1[4]-$_ZA1[5]))."</td><td>&nbsp;</td>";
					reset($_TMP);							
					while(list($a,$b)=each($_TMP)) {
						echo "<td class='r'>".numbAcc($_A1[$a][4]-$_A1[$a][5])."</td>";
					}
					echo "<td class='r'>".numbAcc($_ZA1[4]-$_ZA1[5])."</td></tr>";					
					//----------------------------------------------------------------------------
					echo "<tr><td class='bg'><strong>Aset Bersih Akhir Periode :</strong></td>";
					reset($_TMP);							
					while(list($a,$b)=each($_TMP)) {
						echo "<td class='bg r'><strong>".numbAcc(($_N1[$a][4]-$_N1[$a][5])+($_M1[$a][4]-$_M1[$a][5])+($_A1[$a][4]-$_A1[$a][5]))."</strong></td>";
					}
					echo "<td class='bg r'><strong>".numbAcc(($_ZN1[4]-$_ZN1[5])+($_ZM1[4]-$_ZM1[5])+($_ZA1[4]-$_ZA1[5]))."</strong></td><td>&nbsp;</td>";
					reset($_TMP);							
					while(list($a,$b)=each($_TMP)) {
						echo "<td class='bg r'><strong>".numbAcc(($_M1[$a][4]-$_M1[$a][5])+($_A1[$a][4]-$_A1[$a][5]))."</strong></td>";
					}
					echo "<td class='bg r'><strong>".numbAcc(($_ZM1[4]-$_ZM1[5])+($_ZA1[4]-$_ZA1[5]))."</strong></td></tr>";
				}	     
			?>
             </table><br />&nbsp;
        </td>
