		...
		if($_PT['tSimpan']) {
			...
			if(!$errtNama && !$errtAlamat) {				
				if($tId) {
					...
					$tIdDonatur=substr(((getValue("id_donatur","t_donatur","1=1 order by id_donatur desc limit 0,1")*1)+1001),1,3);
					queryDb("insert into t_donatur(id_donatur,nama,alamat,tanggal) values('".$tIdDonatur."','".$tNama."','".$tAlamat."','".TANGGAL."')");
				}
				
				header("location:?edit=".bs64_e($tIdDonatur));
				exit;
			}
		}
?>
...
<div id="titleTop">
	...
	<ol class="title">
    	<li>NO</li>
        <?php        
        $listTitle=array("d.id_donatur#ID Donatur","d.nama#Nama","d.alamat#Alamat");
...
<div id="list">
	...
	$row=50;
	$page=8;
	$sql="select d.id_donatur,d.nama,d.alamat,if(min(p.id_program) is null,1,0) as stat_delete from t_donatur d
	...
	if(is_array($paging)) {		
		...
		while($b=mysql_fetch_array($a)) {
			...
			echo "<ul id=\"".$b['id_donatur']."\" onclick=\"listFocus(this,'edit=1,del=".$b['stat_delete']."')\"><li>".$paging['start'].".</li><li>".$b['id_donatur']."</li><li>".$b['nama']."</li><li>".$b['alamat']."</li></ul>";
