<?php
	...
	$titleHTML=getValue("s.judul","t_user u
											...
											inner join t_menu_sub s on s.id_menu_sub=h.id_menu_sub and s.status='1' and s.page='siswa.php'",
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_PT['tSimpan']) {
			...
			if(!count($errtNominal) && is_array($tNominal)) {			
				queryDb("delete from t_siswa_jns_bayar where id_siswa in (select id_siswa from t_siswa where id_siswa='".$_GT['id1']."' and concat(id_unit1,'.',id_unit2,'.',id_unit3) in ('".implode("','",array_keys($_UNITS))."'))");
				
				$tNominal=array_filter($tNominal);
				
				while(list($a,$b)=each($tNominal)) {
					queryDb("insert into t_siswa_jns_bayar(id_siswa,id_jns_bayar_siswa,nominal) 
									select id_siswa,'".$a."','".clearRupiah($b)."' from t_siswa where id_siswa='".$_GT['id1']."' and concat(id_unit1,'.',id_unit2,'.',id_unit3) in ('".implode("','",array_keys($_UNITS))."')");
				}
				
				header("location:siswa.php?edit=".bs64_e($_GT['id1'])."&hal=".bs64_e($_GT['hal'])."&sort=".bs64_e($_GT['sort'])."&search=".bs64_e($_GT['search'])."&order=".bs64_e($_GT['order1']));
				exit;
			}
		}
?>
...
<div id="titleTop">
  	<ol><li><?=strtoupper($titleHTML)." ".getValue("concat('[',id_siswa,'] ',nama)","t_siswa","id_siswa='".$_GT['id1']."' limit 1")?></li></ol>
    <ol class="tab"><li>
    	<a href="siswa.php?edit=<?=bs64_e($_GT['id1'])."&hal=".bs64_e($_GT['hal'])."&sort=".bs64_e($_GT['sort'])."&search=".bs64_e($_GT['search'])."&order=".bs64_e($_GT['order1'])?>" class="on"><?=strtoupper($_IST['siswa.php'])?></a>
...
<div id="list">
	...
	$a=queryDb("select j.id_jns_bayar_siswa,j.nama,ifnull(b.nominal,0) as nominal from t_jns_bayar_siswa j
						left join t_siswa_jns_bayar b on b.id_jns_bayar_siswa=j.id_jns_bayar_siswa and b.id_siswa in (select id_siswa from t_siswa where id_siswa='".$_GT['id1']."' and concat(id_unit1,'.',id_unit2,'.',id_unit3) in ('".implode("','",array_keys($_UNITS))."'))
