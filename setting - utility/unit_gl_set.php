<?php
	...
	$titleHTML=getValue("s.judul","t_user u
											...
											inner join t_menu_sub s on s.id_menu_sub=h.id_menu_sub and s.status='1' and s.page='unit_gl.php'",
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_PT['tSimpan']) {			
			...
			queryDb("delete from t_glu where saldo_awal=0 and mutasi_d=0 and mutasi_c=0 and saldo=0 and concat(id_unit1,'.',id_unit2,'.',id_unit3)='".$_GT['edit']."'");
			...
			queryDb("insert into t_glu(id_unit1,id_unit2,id_unit3,id_gl1,id_gl2,id_gl3,id_gl4,id_aruskas,id_aruskas_sub,nama,saldo_awal,mutasi_d,mutasi_c,saldo,tanggal)
						select u.id_unit1,u.id_unit2,u.id_unit3,g.id_gl1,g.id_gl2,g.id_gl3,g.id_gl4,g.id_aruskas,g.id_aruskas_sub,g.nama,'0','0','0','0','".TANGGAL2."'
								from t_gl4 g
									inner join t_unit3 u on concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3)='".$_GT['edit']."'
								 where g.id_gl4 in ('".implode("','",$tGl)."')");
			
			header("location:unit_gl.php?edit=".bs64_e($_GT['edit']));
			exit;
		}
		...
		$sql="select g.id_gl1,g.id_gl2,g.id_gl3,g.id_gl4,g1.nama as nama1,g2.nama as nama2,g3.nama as nama3,g.nama as nama4,if(u.id_gl4 is null,0,1) as checked,if(u.saldo_awal<>0 or u.mutasi_d<>0 or u.mutasi_c<>0 or u.saldo<>0,1,0) as disabled
						from t_gl4 g
							...
							left join t_glu u on u.id_gl1=g.id_gl1 and u.id_gl2=g.id_gl2 and u.id_gl3=g.id_gl3 and u.id_gl4=g.id_gl4 and concat(u.id_unit1,'.',u.id_unit2,'.',u.id_unit3)='".$_GT['edit']."'
		...
		while($b=mysql_fetch_array($a)) {
			...
			$_S[$b['id_gl1']][$b['id_gl2']][$b['id_gl3']][$b['id_gl4']]="<input type=\"checkbox\" ".((!$b['disabled'])?"name=\"tGl[]\" value=\"".$b['id_gl4']."\" onclick=\"uncheckAll('tGl[]');\"":"disabled=\"disabled\"")." ".$checked." />";
		}
?>
...
<div id="titleTop">
    ...
    <ol class="tab"><li>
    	<a href="unit_gl.php?edit=<?=bs64_e($_GT['edit'])?>" class="on">UNIT KERJA</a>
