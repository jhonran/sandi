<?php
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		if($_PT['tSimpan']) {			
			...
			queryDb("update t_gl4 set id_aruskas='',id_aruskas_sub='' where id_aruskas='".$_GT['id1']."' and id_aruskas_sub='".$_GT['id2']."'");
			
			queryDb("update t_gl4 set id_aruskas='".$_GT['id1']."',id_aruskas_sub='".$_GT['id2']."' where id_gl4 in ('".implode("','",$tGl)."') and id_gl1<>'3'");
			queryDb("update t_glu t
								inner join t_gl4 g on g.id_gl4=t.id_gl4
							set t.id_aruskas=g.id_aruskas,t.id_aruskas_sub=g.id_aruskas_sub");
			queryDb("update t_glt t
								inner join t_gl4 g on g.id_gl4=t.id_gl4
							set t.id_aruskas=g.id_aruskas,t.id_aruskas_sub=g.id_aruskas_sub");
			
			header("location:arus_kas_gl_sub.php?edit=".bs64_e($_GT['id2'])."&order=".bs64_e($_GT['order2'])."&id1=".bs64_e($_GT['id1'])."&hal=".bs64_e($_GT['hal'])."&sort=".bs64_e($_GT['sort'])."&search=".bs64_e($_GT['search'])."&order1=".bs64_e($_GT['order1']));
			exit;
		}
		
		
        $statCheckAll=1;
		$sql="select g.id_gl1,g.id_gl2,g.id_gl3,g.id_gl4,g1.nama as nama1,g2.nama as nama2,g3.nama as nama3,g.nama as nama4,if(g.id_aruskas='' or g.id_aruskas_sub='',0,1) as checked
						from t_gl4 g
						...
						where g.id_gl1<>'3' and ((g.id_aruskas='".$_GT['id1']."' and g.id_aruskas_sub='".$_GT['id2']."') or g.id_aruskas='')
...
<div id="titleTop">
  	<ol><li><?=strtoupper($titleHTML)." ".getValue("nama","t_aruskas_sub","id_aruskas='".$_GT['id1']."' and id_aruskas_sub='".$_GT['id2']."' limit 1")?></li></ol>
    <ol class="tab"><li>
        ...
        <a href="arus_kas_gl_sub.php?edit=<?=bs64_e($_GT['id2'])."&order=".bs64_e($_GT['order2'])."&id1=".bs64_e($_GT['id1'])."&hal=".bs64_e($_GT['hal'])."&sort=".bs64_e($_GT['sort'])."&search=".bs64_e($_GT['search'])."&order1=".bs64_e($_GT['order1'])?>" class="on">KATEGORI AKTIFITAS</a>
