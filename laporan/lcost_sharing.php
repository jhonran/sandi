<?php
	...
	if(!$_SESSION['user'] || $_SESSION['waktu']<time() || !$titleHTML) {
		...
		$sql="select u3.id_unit1,u1.nama as nama1,u3.id_unit2,u2.nama as nama2,u3.id_unit3,u3.nama as nama3 
											...
											where concat(u3.id_unit1,'.',u3.id_unit2,'.',u3.id_unit3) in (select distinct concat(id_unit1,'.',id_unit2,'.',id_unit3) from t_costsharing)
		...
		while($b=mysql_fetch_array($a)) {
			...
			$_S[$b['id_unit1']][$b['id_unit2']][$b['id_unit3']]="<input type=\"radio\" name=\"tUnit[]\" value=\"".$b['id_unit1'].".".$b['id_unit2'].".".$b['id_unit3']."\" />";
		}
?>
...
<script language="javascript">
	...
	function viewLap() {
		...
		popUp('../popup/v_lcost_sharing.php?unit='+bs64_e(units.join("#"))+'&jenis='+bs64_e(elm('tJenisLap').value)+'&periode1='+bs64_e(elm('tTanggal').value)+'&periode2='+bs64_e(elm('tTanggal2').value)+'&bulan='+bs64_e(elm('tBulan').value)+'&tahun='+bs64_e(elm('tTahun').value)+'&level='+bs64_e(elm('tLevel').value),1000,600,'x');
		return false;
	}
