...
<div id="titleBottom">
	<ol>
    	<li style="width:auto;" class="l">
        	SEBELUMNYA :  
            <input type="text" name="tAwal" id="tAwal" maxlength="10" onkeydown="return DateFormat(this,event.keyCode)" autocomplete="off" required="required" style="width:147px;" />
        </li>
    	<li style="width:auto;" class="l">
        	PERIODE per : 
    	  ...
    	  <button class="icon-search " onclick="viewPosting();">DAFTAR TUTUP BUKU</button>
        </li>
    	<li class="r" style="width:auto;"><input type="checkbox" name="tDetail" id="tDetail" value="2" />  </li>
    	<li class="l" style="width:auto;">PILIH DETAIL</li>
...
<script language="javascript">
	function setTanggal(v,v2,v3) {
		$(function() {
			$("#tAwal").datepicker();
			$("#tAwal").datepicker("option","dateFormat","dd/mm/yy");
			//$("#tAwal").datepicker("setDate",v);
		});
		
		/*$(function() {
			$("#tTanggal").datepicker();
			$("#tTanggal").datepicker("option","dateFormat","dd/mm/yy");
			$("#tTanggal").datepicker("setDate",v2);
		});*/
		
		$(function() {
			$("#tTanggal2").datepicker();
			$("#tTanggal2").datepicker("option","dateFormat","dd/mm/yy");
			$("#tTanggal2").datepicker("setDate",v3);
		});
	}
	
	setTanggal('1/1/<?=(date("Y")-1)?>','<?=date("1/1/Y")?>','<?=date("d/m/Y")?>');
	...
	function viewLap() {
		...
		popUp('../popup/v_laporan_posisi_keuangan'+((elm('tAwal').value)?"_komper":"")+((elm('tDetail').checked)?elm('tDetail').value:"")+'.php?unit='+bs64_e(units.join("#"))+'&awal='+bs64_e(elm('tAwal').value)+'&periode='+bs64_e(elm('tTanggal2').value),1000,600,'x');
		return false;
	}
	...
	function viewLap2() {
		...
		if(windowPopUp['x']) windowPopUp['x'].close();
		popUp('../popup/tandatangan_lap.php?x=4&unit='+bs64_e(units.join("#"))+'&awal='+bs64_e(elm('tAwal').value)+'&periode='+bs64_e(elm('tTanggal2').value),1000,600,'x');
		return false;
	}
	
	function viewPosting() {
		if(windowPopUp['x']) windowPopUp['x'].close();
		popUp('../popup/tutup_buku.php',1000,600,'x');
		return false;
	}
