...
function valTanggal(angka) {	 
	 var xx=doGetCaretPosition(angka);
	 
	 var isi = angka.value.split("");
	 var hasil='';
	 var hari='';
	 var bulan='';
	 var tahun='';
	 ...
	 for(var i=0;i<isi.length;i++) {	 	
		if(parseFloat(isi[i]) || parseFloat(isi[i])==0 || isi[i]=='/') {
			if(i==2 || i==5) {
				if(isi[i]!='/') {
					hasil+="/"+isi[i];
					xx++;
				...
				if(i==0) {
					...
					if(bulan==0) {
						...
						else if(hari<31 && isi[i]>0 && isi[i]!=2) {
							hasil+=isi[i];
							bulan+=isi[i];
						}
						else if(hari<32 && (isi[i]==1 || isi[i]==3 || isi[i]==5 || isi[i]==7 || isi[i]==8)) {
							hasil+=isi[i];
							bulan+=isi[i];
						}
					}
						...
						else if(hari<32 && (isi[i]==0 || isi[i]==2)) {
							hasil+=isi[i];
							bulan+=isi[i];
						}
					}
					...
					var kabisat=tahun+isi[i];
					if((hari==29 && bulan==02 && kabisat%4==0) || hari!=29 || bulan!=02) hasil+=isi[i];
				}
				else {
					hasil+=isi[i];
					tahun+=isi[i];
              ...
              <table cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <input type="text" name="tTanggal" id="tTanggal" maxlength="10" onkeydown="return DateFormat(this,event.keyCode)" autocomplete="off" required="required" style="width:147px;" onkeyup="valTanggal(this);hideFade('errtTanggal')" />
